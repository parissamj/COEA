...
def get_BC_facility_links():
	...
	facility_links_location = map_to_drive() + "/Project Data_new/BCOGC/BC_facility_linkage.csv"
				...
				if row[FROMWANUM_index] != 'NULL':
					if row[fac_subtype] == 'GP': #excludein include certain facility types 
						if row[FROMWANUM_index] not in BC_facility_links:
							BC_facility_links[int(row[FROMWANUM_index])] = [row]
						else:
							BC_facility_links[int(row[FROMWANUM_index])].append(row)
	...
	print("BC facility link file is:", BC_facility_links)
	print('\n')
...
def get_BC_facility_list(well_data, well_data_headers, BC_facility_links, BC_facility_links_headers, BC_facility_index, BC_facility_index_headings):
	...
	if len(well_data) > 0:
		...
		#print ("Wells are:", well_WA)
		#print ("UWIs are:", WA_to_UWI)
		...
		for WA in WA_to_UWI:
			...
			if WA in BC_facility_links:
				...
				#print("-------------------------CONNECTED WELSS:", connected_WA)
				#we also want a list of the connected facilities
				for i in range(0,len(BC_facility_links[WA])):
					#print ("facility links", BC_facility_links)
					facility = int(BC_facility_links[WA][i][FACILITYID_index])
					#print("facility, facilityID, fac_typeindex=", facility, FACILITYID_index, fac_type_index)
					
					#Line 102 was in the original code. I added the try-except lines.
					#print("size0=", len(BC_facility_index))
					#print("size1=", len(BC_facility_index[0]))
					#print("size2=", len(BC_facility_index[0][0]))
					#print("index=",BC_facility_index[0][0][0])
					
					#try:
					fac_type = BC_facility_index[facility][0][fac_type_index]
					#except KeyError:
						#pass
						#print(" BAD INDEX IS " + str(facility) + " fac_type index is: " + str(fac_type_index))
						#print("try 1 " + str(BC_facility_index[facility][0][0]))
						#exit(1)
		...
		#print ("list of connected wells:", connected_WA, "list of connected facilities:", BC_facility_list)	
	...
	if len(well_data) == 0:
		...
		for WA in BC_facility_links:
			...
			if WA not in connected_WA:
				connected_WA.append(WA) #append to connected WA list
				#print("------------------CONNECTED WELLS:", connected_WA)
...
def get_BC_facility_data(date_array, BC_facility_list):
	...
	for year in years_array:
		...
		BC_facility_data_path = map_to_drive() + "/Project Data_new/BCOGC/facility_volumetrics/" + year + ".csv"
		...
		with open(BC_facility_data_path) as f:
			...
			for row in reader:
				...
				if switch == 1:
					...
					FAC_ID_CODE = int(row[FAC_ID_CODE_index])
					#Format Date
					...
					if FAC_ID_CODE in BC_facility_list: #comment this for ALL facilites?
						...
						if year_month in date_array:
							...
							for volume in range(1,len(row) -1):
								...
								#print ("ffffffffffffffffffffffffff", BC_facility_data[FAC_ID_CODE][year_month][activity_product])
...
def BC_facility_data_summary(BC_facility_data, BC_facility_data_headings, date_array):
	...
	for facility in BC_facility_data:
		if facility != 'ALL': #dont want to double count
			...
			print("facility:", facility, "YearMonth:", year_month, "Activity product:", activity_product, "Total:", BC_facility_data['ALL'][year_month][activity_product])
...
def get_BC_facility_index():
	...
	facility_index_location = map_to_drive() + "/Project Data_new/BCOGC/facindex.csv"
	...
	with open(facility_index_location) as f:
		...
		for row in reader:
			...
			if switch == 1:
				FAC_ID = int(row[FAC_ID_CODE_index])
				#print("FAC_ID = '" + str(FAC_ID) + "', column = " + str(FAC_ID_CODE_index))
...
if __name__ == '__main__':
	...
	date_array = dates_array('2019-01','2019-12')
	field_name = 'poop'
