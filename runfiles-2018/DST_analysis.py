...
def DST_analysis(well_data, well_data_headings, DST_data, DST_headings, OPGEE_data):
	...
	for well in well_data:
		...
		if well in DST_data:
			...
			for test in range(0,len(DST_data[well])):
				...
				if len(DST_test_date) == 0:
					print "Well = " + str(well) + ", test = " +str(test)
					continue
				#print(DST_test_date, date_format1)
	...
	if len(wells_with_DST) > 0:
		...
		ask_pressure_value = 'N' ###########str(raw_input('Would you like to use the calculated pressure gradient (' + str(round(pressure_gradient,3)) + 'psi/ft) ? \nIf no the default pressure gradient is assumed (0.45 psi/ft)\n\nYes/No?   '))
